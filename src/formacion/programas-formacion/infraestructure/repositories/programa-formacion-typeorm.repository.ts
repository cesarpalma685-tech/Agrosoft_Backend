...
import { ProgramaFormacionOrmEntity } from "../persistence/programa-formacion.orm-entity";
...
export class ProgramaFormacionTypeOrmRepository extends ProgramaFormacionRepository {
  ...
  async crear(programa: ProgramaFormacion): Promise<ProgramaFormacion> {
    const orm = this.repo.create(this.aOrm(programa));
  ...
  async actualizar(programa: ProgramaFormacion): Promise<ProgramaFormacion> {
    const actualizado = await this.repo.save(this.aOrm(programa));
  ...
  async buscarPorId(id: number): Promise<ProgramaFormacion | null> {
    ...
    return programa ? this.aDominio(programa) : null;
  }

  private aDominio(orm: ProgramaFormacionOrmEntity): ProgramaFormacion {
