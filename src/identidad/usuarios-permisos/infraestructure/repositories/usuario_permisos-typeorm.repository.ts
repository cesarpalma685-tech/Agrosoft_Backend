...
import { UsuarioPermisoOrmEntity } from "../persistence/usuario_permisos.orm-entity";
...
export class UsuarioPermisoTypeOrmRepository extends UsuarioPermisoRepository {
  ...
  async buscarPorId(id: number): Promise<UsuarioPermiso | null> {
    ...
    return usuarioPermiso ? this.aDominio(usuarioPermiso) : null;
  }

  private aDominio(orm: UsuarioPermisoOrmEntity): UsuarioPermiso {
    return new UsuarioPermiso(orm.id, orm.usuarioId, orm.permisoId);
  }
